#!/usr/bin/env python3
"""PRISMA Crypto D1 publisher v3 — multi-asset parity payload.
...
PAYLOAD_BUILDER = FORK / 'TAC_CLOUDFLARE_PAYLOAD_V3.py'
DEFAULT_INTERVAL = 60
...
def d1_query(sql: str, params: list[Any] | None = None, *, account_id=None, database_id=None, token=None, timeout=30):
    ...
    req = urllib.request.Request(endpoint, data=body, method='POST', headers={
        ...
        'User-Agent': 'PRISMA-Crypto-Colab-D1/3.0',
    })
    try:
...
def publish_latest(payload: dict | None = None) -> dict:
    ...
    print(f"[PRISMA D1 V3] latest OK · hash={payload.get('state_hash')} · {size_kb:.1f}KB · PARITY")
    return out
...
def publish_history(payload: dict) -> dict:
    ...
    print(f"[PRISMA D1 V3] history OK · hash={payload.get('state_hash')}")
    return out
...
def loop(interval_seconds=DEFAULT_INTERVAL, history_interval_seconds=DEFAULT_HISTORY_INTERVAL):
    ...
    next_history = 0.0
    print(f'[PRISMA D1 V3] loop {interval_seconds}s · history {history_interval_seconds}s · PARITY · NO ORDERS')
    while True:
        started = time.time()
        try:
            ...
            print(f'[PRISMA D1 V3] ERROR {type(exc).__name__}: {exc}')
