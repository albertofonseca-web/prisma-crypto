#!/usr/bin/env python3
"""PRISMA Crypto rich payload builder v6 — full TAC/Source event timeline.
...
FULL_EVENT_DISPLAY = FORK / 'TAC_FULL_EVENT_DISPLAY_V1.csv'
...
def _shared_pivots(reference_utc: str | None, *, next_count: int = 24, recent_count: int = 12, timeline_days: int = 14) -> dict:
    """Display calendar separated from execution eligibility.

    - `next`: classified Source/TAC events only, for compatibility with existing
      decision cards and engine semantics.
    - `timeline`: every known display event in the forward horizon, including
      unclassified astronomical candidates marked decision_effect=NONE.
    - Candidates never modify BTC/ETH/XRP scoring, gates, entries, SL or TP.
    """
    ...
    try:
        with path.open('r', encoding='utf-8', newline='') as f:
            for r in csv.DictReader(f):
                ...
                display_class=(r.get('display_class') or 'CLASSIFIED_SOURCE').strip()
                item={
                    ...
                    'decision_effect': r.get('decision_effect') or ('NONE' if display_class == 'CANDIDATE' else 'DISPLAY_CONTEXT'),
                    'verification_status': r.get('verification_status'),
                    'observational_only': display_class == 'CANDIDATE',
                    '_t': t,
                }
                rows.append(item)
    except Exception:
    ...
    future_classified=[x for x in future_all if x.get('display_class') != 'CANDIDATE'][:next_count]

    def clean(seq):
        return [{k:v for k,v in x.items() if k!='_t'} for x in seq]

    timeline=clean(future_all)
    classified=clean(future_classified)
    recent=clean(past)
    ...
    return {
        ...
        'next_classified': classified[0] if classified else None,
        ...
        'calendar_scope': 'FULL_TAC_SOURCE_DISPLAY_WITH_CANDIDATES',
        'decision_effect': 'CANDIDATES_NONE_CLASSIFIED_CONTEXT_ONLY',
        'coverage': {
            'timeline_days': timeline_days,
            'timeline_count': len(timeline),
            'classified_count': sum(1 for x in timeline if x.get('display_class') != 'CANDIDATE'),
            'candidate_count': sum(1 for x in timeline if x.get('display_class') == 'CANDIDATE'),
...
def _btc_rich(asset: dict):
    ...
    return {
        ...
        'pivots': (lambda shared: {
            ...
            'calendar_scope': 'BTC_SOURCE_ENGINE_PLUS_FULL_DISPLAY_TIMELINE',
...
def build_payload() -> dict:
    ...
    payload['publisher_version'] = '6.0.0_FULL_EVENT_TIMELINE'
    payload['terminal_contract'] = 'PRISMA_CRYPTO_RICH_V5_FULL_EVENT_TIMELINE'
