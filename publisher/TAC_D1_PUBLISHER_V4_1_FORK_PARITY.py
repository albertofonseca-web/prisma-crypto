#!/usr/bin/env python3
"""PRISMA Crypto D1 publisher v4 — full fork display parity payload.
...
PAYLOAD_BUILDER = FORK / 'TAC_CLOUDFLARE_PAYLOAD_V4_1_FORK_PARITY.py'
DEFAULT_INTERVAL = 60
...
def d1_query(sql: str, params: list[Any] | None = None, *, account_id=None, database_id=None, token=None, timeout=30):
    ...
    req = urllib.request.Request(endpoint, data=body, method='POST', headers={
        ...
        'User-Agent': 'PRISMA-Crypto-Colab-D1/4.0',
    })
    try:
...
def publish_latest(payload: dict | None = None) -> dict:
    ...
    print(f"[PRISMA D1 V4] latest OK · hash={payload.get('state_hash')} · {size_kb:.1f}KB · PARITY")
    return out
...
def publish_history(payload: dict) -> dict:
    ...
    print(f"[PRISMA D1 V4] history OK · hash={payload.get('state_hash')}")
    return out
...
def loop(interval_seconds=DEFAULT_INTERVAL, history_interval_seconds=DEFAULT_HISTORY_INTERVAL):
    ...
    next_history = 0.0
    print(f'[PRISMA D1 V4] loop {interval_seconds}s · history {history_interval_seconds}s · PARITY · NO ORDERS')
    while True:
        started = time.time()
        try:
            ...
            print(f'[PRISMA D1 V4] ERROR {type(exc).__name__}: {exc}')
...
def start_background(interval_seconds=DEFAULT_INTERVAL, history_interval_seconds=DEFAULT_HISTORY_INTERVAL):
    th = threading.Thread(target=loop, args=(interval_seconds, history_interval_seconds), daemon=True, name='prisma-crypto-d1-publisher-v3')
    th.start(); return th
